...
function defs(){
return '<defs>'+
'<marker id="arrow-auth" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 z" fill="#59ddfa"/></marker>'+
'<marker id="arrow-runtime" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 z" fill="#68eaa9"/></marker>'+
'<marker id="arrow-dormant" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 z" fill="#75694a"/></marker>'+
'<marker id="arrow-conflict" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 z" fill="#ff7079"/></marker>'+
'</defs>';
}
function draw(){
...
p.setAttribute('marker-end',`url(#arrow-${t})`);
p.dataset.from=a;p.dataset.to=b;p.dataset.type=t;p.dataset.label=label;
svg.appendChild(p);

const l=document.createElement('div');
l.className='edge-label';l.textContent=label;l.dataset.from=a;l.dataset.to=b;
l.style.left=((p1.x+p2.x)/2/1200*100)+'%';l.style.top=((p1.y+p2.y)/2/620*100)+'%';
...
scenario:'ENTERPRISE GRAPH',caption:'Dados, software, identidade e agentes no mesmo modelo',
...
score:'5 / 5 sinais',
evidence:[['Declarado','MCP tool ligado à API','yes','SIM'],['Estático','Service referencia customers','yes','SIM'],['Autorização','scope customer.read','yes','SIM'],['Runtime','183 chamadas observadas','yes','SIM'],['Lineage','customers contém cpf','yes','SIM']],
...
stage:['FUSÃO DE EVIDÊNCIAS','Uma relação pode ser possível, autorizada, observada — ou conflitante.'],
notes:['Comece pelo problema','Cada ferramenta conhece uma parte da arquitetura. A pergunta é se correlacionar essas evidências melhora a análise.'],
hot:['cpf'],pairs:[],labels:'all'
},
can:{
...
scenario:'AUTHORIZED REACHABILITY',caption:'Sales Agent → customers.cpf',
...
score:'4 / 5 sinais',
evidence:[['Declarado','Agent usa get_customer','yes','SIM'],['Estático','tool mapeia rota /customers','yes','SIM'],['Autorização','scope customer.read','yes','SIM'],['Runtime','não é necessário p/ CAN IT','unknown','N/A'],['Lineage','Service → table → cpf','yes','SIM']],
...
stage:['AUTHORIZED PATH','O caminho só é válido porque topologia e permissão concordam.'],
notes:['Explique reachability','Mostre que não basta existir uma conexão técnica. A identidade precisa ter permissão para atravessar o caminho.'],
hot:['sales-agent','sales-role','customer-mcp','customer-api','customer-service','customers-table','cpf'],
pairs:[['sales-agent','sales-role'],['sales-role','customer-mcp'],['customer-mcp','customer-api'],['customer-api','customer-service'],['customer-service','customers-table'],['customers-table','cpf']],labels:'active'
},
did:{
...
scenario:'RUNTIME EVIDENCE',caption:'Caminho observado em produção',
...
score:'5 / 5 sinais',
evidence:[['Declarado','get_customer configurado','yes','SIM'],['Estático','API → Customer Service','yes','SIM'],['Autorização','customer.read válido','yes','SIM'],['Runtime','183 chamadas + 179 reads','yes','SIM'],['Lineage','table contém cpf','yes','SIM']],
...
stage:['OBSERVED IN PRODUCTION','As linhas verdes animadas representam dependências efetivamente vistas em runtime.'],
notes:['Destaque “possível ≠ observado”','A permissão mostra o que pode acontecer. O runtime mostra o que realmente aconteceu. São fatos diferentes.'],
hot:['sales-agent','customer-mcp','customer-api','customer-service','customers-table','cpf'],
pairs:[['customer-mcp','customer-api'],['customer-api','customer-service'],['customer-service','customers-table'],['customers-table','cpf']],labels:'active'
},
breaks:{
...
scenario:'CHANGE IMPACT',caption:'Simulação: cpf muda de string → integer',
...
score:'4 / 5 sinais',
evidence:[['Declarado','3 consumidores conhecidos','yes','SIM'],['Estático','Analytics Agent referencia API','yes','SIM'],['Autorização','Sales Agent autorizado','yes','SIM'],['Runtime','2 consumidores ativos','yes','SIM'],['Lineage','API alcança customers.cpf','yes','SIM']],
...
stage:['SIMULATED CHANGE','cpf: string → integer · 2 consumidores ativos · 1 dependência dormente'],
notes:['Mostre a priorização','O lineage poderia listar três consumidores. O runtime ajuda a separar dois ativos de um consumidor dormente.'],
hot:['customer-api','customer-mcp','sales-agent','crm-app','analytics-agent'],
pairs:[['customer-mcp','customer-api'],['customer-api','crm-app'],['analytics-agent','customer-api']],labels:'active'
},
hidden:{
...
scenario:'EVIDENCE CONFLICT',caption:'Legacy Exporter apareceu em runtime, mas não existe no catálogo',
...
score:'1 conflito',
evidence:[['Declarado','nenhuma relação encontrada','no','NÃO'],['Estático','serviço não mapeado','no','NÃO'],['Autorização','identidade não resolvida','unknown','UNKNOWN'],['Runtime','read observado há 12m','warn','SIM'],['Lineage','customers é fonte do serviço','warn','INFERIDO']],
...
stage:['ARCHITECTURE DRIFT','Runtime observou uma dependência que não aparece no catálogo nem na análise estática.'],
notes:['Feche com o conflito','Essa é a parte mais “pesquisa”: fontes discordam. Em vez de apagar a divergência, o Evidence Graph a transforma em sinal.'],
hot:['legacy-service','customers-table'],pairs:[['legacy-service','customers-table']],labels:'active'
}
};

function renderEvidence(items){
document.getElementById('evidenceMatrix').innerHTML=items.map(([type,detail,kind,label])=>`<div class="evidence-row"><span class="ev-type">${type}</span><span class="ev-detail">${detail}</span><span class="signal ${kind}">${label}</span></div>`).join('');
}
...
currentMode=mode;const c=D[mode]||D.overview;
document.querySelectorAll('[data-mode]').forEach(x=>x.classList.toggle('active',x.dataset.mode===mode));
document.getElementById('title').textContent=c.title;document.getElementById('subtitle').textContent=c.subtitle;
document.getElementById('scenarioLabel').textContent=c.scenario;document.getElementById('caption').textContent=c.caption;
document.getElementById('detailTitle').textContent=c.detail;document.getElementById('kicker').textContent=c.kicker;
document.getElementById('badge').textContent=c.badge;document.getElementById('detailCopy').textContent=c.copy;
document.getElementById('insight').textContent=c.insight;document.getElementById('evidenceScore').textContent=c.score;
setMetrics(c.metrics);renderEvidence(c.evidence);setAnswer(c.answer);setCallout(c.callout);setStage(c.stage);setNotes(c.notes);
...
if(n.dataset.id==='legacy-service'){n.style.opacity=mode==='hidden'?'1':'0';n.style.pointerEvents=mode==='hidden'?'auto':'none'}
if(mode!=='overview'&&!c.hot.includes(n.dataset.id))n.classList.add('dim');
if(c.hot.includes(n.dataset.id))n.classList.add(mode==='did'?'hot-runtime':mode==='breaks'?'hot-break':mode==='hidden'?'hot-conflict':'hot');
});
[...svg.querySelectorAll('.edge')].forEach(e=>{
e.classList.remove('dim','hot');
const active=(c.pairs||[]).some(([a,b])=>e.dataset.from===a&&e.dataset.to===b);
if(mode!=='overview'&&!active)e.classList.add('dim');if(active)e.classList.add('hot');
});
document.querySelectorAll('.edge-label').forEach(l=>{
const active=(c.pairs||[]).some(([a,b])=>l.dataset.from===a&&l.dataset.to===b);
l.style.opacity=c.labels==='all'?'.8':active?'1':'0';
});
}
...
n.classList.toggle('open',force===undefined?!n.classList.contains('open'):force);
}
...
document.getElementById('fullscreenBtn').addEventListener('click',toggleFullscreen);document.getElementById('notesBtn').addEventListener('click',()=>toggleNotes());
...
if(/^[1-5]$/.test(e.key))setMode(modeOrder[Number(e.key)-1]);
if(e.key.toLowerCase()==='p')togglePresentation();
if(e.key.toLowerCase()==='n')toggleNotes();
if(e.key.toLowerCase()==='f')toggleFullscreen();
if(e.key==='Escape')toggleNotes(false);
});
nodes.forEach(n=>n.addEventListener('click',()=>{if(n.dataset.id==='cpf')setMode('can');if(n.dataset.id==='customer-api')setMode('breaks');if(n.dataset.id==='legacy-service')setMode('hidden')}));
window.addEventListener('resize',()=>{draw();setMode(currentMode)});
draw();setMode('overview');